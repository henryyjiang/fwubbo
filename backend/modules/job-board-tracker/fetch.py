...
def fetch():
    ...
    # Diff against previously seen URLs to find genuinely new postings
    ...
    # Tag each job so the widget can badge new ones
    for j in all_jobs:
        j["is_new"] = j["url"] not in seen_urls

    # Persist the full current set as seen (union keeps jobs that may have dropped
    # off the page temporarily, but cap at 2000 to avoid unbounded growth)
    updated_seen = list((seen_urls | current_urls))[-2000:]

    companies_ok = [c for c in selected if c not in fetch_errors]
    notifications = []

    if new_jobs:
        # Build a concise summary: group by company
        by_company: dict = {}
        for j in new_jobs:
            by_company.setdefault(j["company"], 0)
            by_company[j["company"]] += 1
        company_summary = ", ".join(
            f"{count} at {co}" for co, count in sorted(by_company.items())
        )
        notifications.append({
            "title": f"{len(new_jobs)} new {keywords} job{'s' if len(new_jobs) != 1 else ''}",
