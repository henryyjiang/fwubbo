...
export default function Widget({ data, loading, error, lastUpdated }: WidgetProps) {
  ...
  return (
    <div className="no-drag h-full flex flex-col gap-0 overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-2 px-3 pt-2 pb-1 flex-shrink-0">
        ...
        <span className="text-xs text-text-muted ml-auto flex items-center gap-1.5">
          {(data.new_count ?? 0) > 0 && (
            <span className="text-[10px] font-mono font-bold text-status-ok bg-surface-raised border border-status-ok/30 px-1.5 py-0.5 rounded-full">
              +{data.new_count} new
            </span>
          )}
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((job, i) => (
            <div
              ...
              className={`border rounded-lg px-3 py-2 flex items-start gap-2 group cursor-pointer transition-colors ${
                job.is_new
                  ? "bg-surface-raised border-status-ok/40 hover:border-status-ok"
                  : "bg-surface-raised border-border-subtle hover:border-accent-primary"
              }`}
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                  ...
                  {job.is_new && (
                    <span className="text-[9px] font-mono font-bold text-status-ok uppercase tracking-wider">new</span>
                  )}
