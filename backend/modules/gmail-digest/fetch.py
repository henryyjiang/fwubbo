...
def fetch_emails(address, app_password, days_back, max_count):
    ...
    try:
        cutoff = (datetime.now(timezone.utc) - timedelta(days=days_back)).strftime("%d-%b-%Y")
            ...
            for uid in uids:
                try:
                    ...
                    try:
                        ...
                        date_label = date_str[:16] if date_str else "Unknown"
    ...
    try:
        ...
        if not address or not app_password:
            ...
            return {
                ...
                "data": {},
                "notifications": [],
                "error_message": "Set gmail_address and gmail_app_password in widget settings",
            }
        ...
        if not api_key:
            ...
            return {
                ...
                "data": {},
                "notifications": [],
                "error_message": "ANTHROPIC_API_KEY not configured in backend/.env",
            }
        ...
        try:
            ...
            return {
                ...
                "data": {},
                "notifications": [],
                "error_message": f"Gmail login failed: {e}. Make sure you're using an App Password, not your account password.",
            }
        except Exception as e:
            if cached:
                return {
                    "status": "ok",
                    "data": {**cached, "from_cache": True, "warning": f"Gmail connection failed: {str(e)[:50]}"},
                    "notifications": [],
                }
            return {
                "status": "error",
                "data": {},
                "notifications": [],
                "error_message": f"Could not connect to Gmail: {e}",
            }
...
Return only valid JSON, no markdown fences."""
        ...
        try:
            ...
            return {
                ...
                "data": {},
                "notifications": [],
                "error_message": f"Failed to analyze emails: {e}",
            }
    except Exception as e:
        return {
            "status": "error",
